...
{
	//Initiialize physics world with demostate data
	demoState->physicsWorld->lock = 0;
	demoState->physicsWorld->rate = 200.0;
	demoState->physicsWorld->state->planetCount = demoState->planetCount;
	demoState->physicsWorld->state->planetSpeed = demoState->planetSpeed;

	for (size_t i = 0; i < demoState->planetCount; i++)
	{
		demoState->physicsWorld->state->planetData[i].distance = demoState->planetData[i].distance;
		demoState->physicsWorld->state->planetData[i].earthDay = demoState->planetData[i].earthDay;
		demoState->physicsWorld->state->planetData[i].orbit = demoState->planetData[i].orbit;
		demoState->physicsWorld->state->planetData[i].orbitalPeriod = demoState->planetData[i].orbitalPeriod;
		demoState->physicsWorld->state->planetData[i].size = demoState->planetData[i].size;
	}
